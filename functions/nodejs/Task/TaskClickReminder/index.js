...
module.exports = async function (params, context, logger) {
    ...
    //获取普通任务
    const object_store_tasks = await application.data
        ...
        .find();
    //待发送飞书消息列表
    let messageCardSendDataList = [];
    let taskCount = 0;
    let userCount = 0;
    //查询任务定义
    const task_def_record = await application.data
        .object('object_task_def')
        .select('_id', 'option_upload_image', 'option_input_information', 'option_upload_attachment', 'send_channel')
    ...
    if (object_store_tasks.length > 0) {
        for (const item of object_store_tasks) {
            ...
            //判断执行流程的url
            const url = `https://${tenantDomain}.feishuapp.cn/ae/apps/${namespace}/aadgik5q3gyhw?params_var_bcBO3kSg=${item._id}`;
            const pc_url = `https://${tenantDomain}.feishuapp.cn/ae/apps/${namespace}/aadgik5q3gyhw?params_var_bcBO3kSg=` + item._id;
            const android_url = `https://${tenantDomain}.feishuapp.cn/ae/apps/${namespace}/aadgihlti4uni?params_var_LLsDqf8w=` + item._id;
            const ios_url = `https://${tenantDomain}.feishuapp.cn/ae/apps/${namespace}/aadgihlti4uni?params_var_LLsDqf8w=` + item._id;
            const content = {
                config: {
            ...
            if (item.task_chat) {
                //获取群组ID
                const feishuChat = await application.data.object('object_feishu_chat').select('_id', 'chat_id').where({ _id: item.task_chat._id }).findOne();
                data.receive_id_type = 'chat_id';
                data.receive_id = feishuChat.chat_id;
                messageCardSendDataList.push(data);
                taskCount++;
            } else {
                const feishuPeople = await application.data.object('_user').select('_id', '_name', '_department', '_lark_user_id').where({ _id: item.task_handler._id }).findOne();
                content.header.title.content =
                    '【催办消息】' + feishuPeople._name.find(item => item.language_code === 2052).text + '有一条' + object_store_tasks_name + '门店任务请尽快处理！';
                ...
                if (task_def_record.send_channel === 'option_group') {
                    let object_feishu_chat = await application.data
                        ...
                        .select('_id', 'chat_id')
                        .where({ department: feishuPeople._department._id })
                        .findOne();
