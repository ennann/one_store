...
module.exports = async function (params, context, logger) {
    logger.info(`接收飞书事件函数开始执行 ${new Date()}`,params);
    ...
    switch (event_type) {
        ...
        case 'im.chat.member.user.added_v1':
            ...
            logger.info('本次事件：用户进群事件');
            await faas.function('EventUserAddToChat').invoke(params);
            logger.info('用户进群事件处理完成');
            break;

        case 'im.chat.member.user.deleted_v1':
            // im.chat.member.user.deleted_v1 用户主动退群或被移出群聊
            logger.info('本次事件：用户主动退群或被移出群聊事件');
            await faas.function('EventUserOutToChat').invoke(params);
            logger.info('用户主动退群或被移出群聊事件处理完成');
            break;

        case 'im.chat.member.user.withdrawn_v1':
            // im.chat.member.user.withdrawn_v1 撤销拉用户进群
            logger.info('本次事件：撤销拉用户进群事件');
            await faas.function('EventUserOutToChat').invoke(params);
            logger.info('撤销拉用户进群事件处理完成');
            break;
