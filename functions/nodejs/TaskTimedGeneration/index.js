...
module.exports = async function (params, context, logger) {
    ...
    if (task_def_record.carbon_copy) {
        ...
        if (userList.length > 0) {
            ...
            logger.info(`根据抄送人筛选规则获取的人员总数->${apassDataList.length}, 抄送成功数量->${createApassDataSuccess.length}, 抄送失败数量->${createApassDataFail.length}`);
        }
    }
    ...
    try {
        ...
        try {
            ...
            const content = {
                ...
                elements: [
                    { tag: 'div', text: { content: '任务标题：' + task_def_record.name, tag: 'plain_text' } },
                    { tag: 'div', text: { content: '任务描述：' + task_def_record.description, tag: 'plain_text' } },
                    ...
                    {
                        tag: 'div',
                        text: { content: '任务下发时间：' + dayjs(object_task_create_monitor.task_create_time).add(8, 'hour').format('YYYY-MM-DD HH: mm: ss'), tag: 'plain_text' },
                    },
                        ...
                        actions: [
                            {
                                ...
                                type: 'primary',
                                multi_url: { url: url, pc_url: pc_url, android_url: android_url, ios_url: ios_url },
                            },
                        ],
                    },
                ],
...
async function getTaskDefCopyAndFeishuMessageStructure(userList, task_def_record, object_task_create_monitor, logger) {
    ...
    let department_record = await application.data
        .object('_department')
        .select('_id', '_name')
        .where({ _id: task_def_record.publish_department.id || task_def_record.publish_department._id })
        .findOne();
    //遍历人员
