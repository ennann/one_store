...
module.exports = async function (params, context, logger) {
    ...
    if (task_def_record.carbon_copy) {
        ...
        if (userList.length > 0) {
            ...
            logger.info(`根据抄送人筛选规则获取的人员总数->${aPaaSDataList.length}, 抄送成功数量->${createAPaaSDataSuccess.length}, 抄送失败数量->${createAPaaSDataFail.length}`);
        }
    }
...
async function createStoreTaskEntry(taskDefine, taskBatch, logger, limitedSendFeishuMessage) {
    ...
    try {
        ...
        logger.info(`任务处理记录（任务批次）[${taskBatch._id}]对应的任务定义详情->`, JSON.stringify(taskDefine, null, 2));
        ...
        if (taskDefine.option_handler_type === 'option_01') {
            ...
            logger.info(`群组筛选规则[${taskDefine.chat_rule._id}]返回群数量->`, chatRecordList.length);
            ...
            for (const chatRecordListElement of chatRecordList) {
                const createData = {
                    ...
                    source_department: { _id: department_record._id, _name: department_record._name },
            ...
            logger.info(`人员筛选规则[${taskDefine.user_rule._id || taskDefine.user_rule.id}]返回人员数量->`, userList.length);
        ...
        logger.info(`需要为任务处理记录（任务批次）[${taskBatch._id}], 创建的门店普通任务数量->`, createDataList.length);
        ...
        if (createDataList.length > 0) {
            ...
            logger.info(`为任务处理记录（任务批次）[${taskBatch._id}]创建门店普通任务成功数量: ${successfulStoreTasks.length}, 失败数量: ${failedStoreTasks.length}`);
            ...
            logger.info(`根据门店普通任务创建记录需要发送飞书数量---->${messageCardSendDataList.length}`);
...
async function createStoreTaskEntryStart(storeTask, logger) {
    // storeTask 代表门店普通任务
    try {
        logger.info('门店任务数据 createStoreTaskEntryStart--->', storeTask);
...
async function getTaskDefCopyAndFeishuMessageStructure(userList, taskDefRecord, taskBatch, logger) {
    const cardDataList = [];
    const aPaaSDataList = [];
    logger.info('抄送任务定义详情->', JSON.stringify(taskDefRecord, null, 2), '任务批次详情->', JSON.stringify(taskBatch, null, 2));

    // 获取部门详情
    const department_record = taskDefRecord.publish_department
