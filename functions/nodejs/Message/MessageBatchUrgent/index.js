...
module.exports = async function (params, context, logger) {
    ...
    const getUnReadUsers = async (_id) => {
        ...
        try {
            await DB(READ_OBJECT)
                ...
                .findStream(async (records) => {
                    unreadRecords.push(...records);
                    // await sleep(800);
                });
            return unreadRecords.map(item => item.accept_user);
        }catch (e) {
            logger.error("获取未读用户列表出错", e);
        }
    };
    ...
    const getUrgentList = async () => {
        ...
        if (msgRecords.length > 0) {
            ...
            for (let i = 0; i < msgRecords.length; i += batchSize) {
                let batch = msgRecords.slice(i, i + batchSize);
                const urgent = await Promise.all(batch.map(ele => getUrgent(ele)));
                urgentList = [...urgentList, ...urgent]
            }
            return urgentList;
        }
        return [];
    ...
    try {
        ...
        for (let item of urgentList) {
            await faas.function("MessageUrgent").invoke({ ...item });
        }
