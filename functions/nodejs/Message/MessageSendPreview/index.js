...
module.exports = async function (params, context, logger) {
    ...
    if (record?.option_message_type === "option_card"){
        const cardTemp = await application.data.object('object_message_card_template')
            .select('message_template_id')
            .where({_id: record.message_template?.id})
            .findOne()
        // 数据组装
        record.message_template_id = cardTemp?.message_template_id
    }
