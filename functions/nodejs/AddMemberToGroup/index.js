...
module.exports = async function(params, context, logger) {
    ...
    if (!Array.isArray(emails) || emails.length === 0) {
        logger.error("未提供有效的emails数组");
        response.code = -1;
        response.message = "未提供有效的emails数组或数组为空";
        return response;
    }
    ...
    try {
        ...
        let addMemberRes = await client.im.chatMembers.create({
            path: { chat_id },
        ...
        if (addMemberRes.code !== 0) {
            ...
            response.message = "添加聊天成员失败: " + addMemberRes.msg;
            return response;
        }

        response.message = "聊天成员添加成功";
    } catch (error) {
